<?php
  ...
  class cronJobs {
      ...
      public function import() {
          ...
          $on_hold = $db->selectRow('import_logs', $where);
          if($in_progress) {
              $job_result['res'] = "In Progress:". $in_progress['id'];
              $import = false;
          }
          else if($on_hold) {
              $import = $on_hold;
              ...
              $import = $db->selectRow('import_logs', $where);
          }
          if($import){
              $db->updateId('import_logs',$import['id'], ['status'=>'In Progress']);
              require_once '../import/exlsx.php';

              $target_file = '../import/files/'.$import['file_name'];
              $rows = array();
              if(  in_array($import['type'],['csv','CSV']) ){
              ...
              if($rows) {
                  ...
                  $current_row = $import['imported_rows'];
                  $step_count = 250;
                  if($current_row<$import['file_rows']+1){
                      for ($i = 0; $i < $step_count; ++$i) {
                          $row = $rows[++$current_row];

                          if( !is_array($row) ) break;
                          if($current_row-$step_count > $import['file_rows']) break;
                          $db->increase('import_logs', 'imported_rows', 'id='.$import['id'] );
                          ...
                          if(strlen($row['phone'])<5){
                              // Add Error
                              $db->increase('import_logs', 'error', 'id='.$import['id'] );
                              continue;
                          }
                          ...
                          if($exist_phone) {
                              // Add Phone D
                              $db->increase('import_logs', 'phone_d', 'id='.$import['id'] );
                              continue;
                          }

                          // Email
                          if (filter_var($row['email'], FILTER_VALIDATE_EMAIL)) {
                              $row['email'] = GF::clearEmail($row['email']);
                          } else {
                          ...
                          if($exist) {
                              // Add Email D
                              $db->increase('import_logs', 'email_d', 'id='.$import['id'] );
                              continue;
                          }

                          // Name
                          $row['fname'] = GF::clearString($row['fname']);
                          $row['lname'] = GF::clearString($row['lname']);

                          // Country
                          $row['country'] = GF::clearString($row['country']);

                          // Unit
                          $row['unit'] = $db->selectId('units', $row['unit_id'], 'name')['name'];

                          // Type
                          $row['type'] = $db->selectId('type', $row['type_id'], 'name')['name'];
                          ...
                          $new_user_id = $db->insert('users', $ins_users);
                          if($new_user_id){
                              // Insert - user_extra
                              $ins_extra = array(
                                  "user_id"       => $new_user_id,
                                  ...
                                  "updated_at"    => $date,
                                  "updated_by"    => $import['created_by'],
                                  "language"      => LANGUAGE_NAME
                              );
                              $ins_extra_res = $db->insert('user_extra', $ins_extra);
                              ...
                              $ins_fx_res = $db->insert('user_fx', $ins_fx);

                              // Insert - user_gi
                              $ins_gi = array(
                                  "user_id"       => $new_user_id,
                                  "bd"            => "2000-01-01 12:00:00",
                                  "whatsapp"      => "",
                                  "telegram"      => "",
                                  "facebook"      => "",
                                  "instagram"     => "",
                              ...
                              $ins_gi_res = $db->insert('user_gi', $ins_gi);

                              // Insert - user_marketing
                              $ins_marketing = array(
                                  "user_id"       => $new_user_id,
                                  "lead_src"      => $row['source'],
                                  "lead_camp"     => $row['campaign'],
                              ...
                              $ins_marketing_res = $db->insert('user_marketing', $ins_marketing);
                              ...
                              // Add actLog
                              $db->increase('import_logs', 'success', 'id='.$import['id'] );
                              global $actLog; $actLog->add('New Lead', $new_user_id, 1, json_encode($act_detail));
                          }
                          else {
                              // Add Error
                              $db->increase('import_logs', 'error', 'id='.$import['id'] );
                  ...
                  $status = ($current_row>=$import['file_rows']) ? 'Done' : 'On Hold';
                  $db->updateId('import_logs',$import['id'], ['status'=>$status]);
                  $job_result['res'] = array(
                      "id" => $import['id'],
                      "File" => $import['file_name'],
                      "rows" => $import['file_rows'],
                      "current" => $current_row,
                  );
              }
          }
          else {
              $job_result['res'] = "Queue is empty";
          }
