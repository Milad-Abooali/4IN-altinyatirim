...
<div id="scripts-wrapper">
    ...
    <script>
        let notifications = {
            UI : {
                badge: $(`.notification-list .noti-icon-badge`),
                badgeNotifications: $(`#notifications .noti-icon-badge`),
                badgeFollowups: $(`#followups .noti-icon-badge`),
                general: $(`#right-bar #general-notifications`),
                financial: $(`#right-bar #financial-notifications`),
                todo: $(`#right-bar #todo-notifications`)
            },
            countAll: 0,
            cats: {
                financial: {
        ...
        function getNotifications(interval=true) {
            ajaxCall ('notify', 'getNotify','', function(response){
                ...
                if(resObj.e) {
                    ...
                    handelNotifications(resObj.res);
                    if(interval) {
                        setTimeout( getNotifications, 20*1000 );
                    }
                }
            });
        }
        function handelNotifications(items){
            let counter = Object.keys(items).length;
            notifications.countAll = counter;
            notifications.cats.financial.count=0;
            notifications.cats.todo.count=0;
            notifications.cats.general.count=0;
            notifications.cats.financial.items='';
            notifications.cats.todo.items='';
            notifications.cats.general.items='';
            notifications.UI.badgeNotifications.html(notifications.countAll).fadeIn();
            for (const item in items) {
                const notify = items[item];
                if(  typeof notify !== 'undefined') {
                    notifications.cats[notify.cat].count++;
                    notifications.UI.badgeFollowups.html(notifications.cats.todo.count);
                    // check if not exist
                    if($(`#notify-${notify.id}`).length>0) continue;

                    notifications.cats[notify.cat].items += `<a id="notify-${notify.id}" `+
                    `href="javascript:void(0);" data-id="${notify.id}" data-cat="${notify.cat}"`+
                    ` data-user="${notify.notify_data}" class="dropdown-item notify-item detail active"> `+
                    `<div class="notify-icon bg-${notifications.cats[notify.cat].color}"> `+
                    `<i class="mdi mdi-${notifications.cats[notify.cat].icon}"></i></div>`;

                    if(notify.cat === 'financial'){
                        notifications.cats[notify.cat].items += `<p class="notify-details">${notify.cat} `+
                        `<span class="text-muted"><b>${notify.user}</b> requested `+
                        `<b>$${notify?.details?.amount}</b> ${notify?.details?.type}.</span></p></a>`;
                    }
                    else {
                        let pText = (notify.hasOwnProperty(`n_text`)) ? notify.n_text.replace('%s', `<b>${notify.notify_data}</b>` ) : '';
                        notifications.cats[notify.cat].items += `<p class="notify-details">${notify.cat}`+
                        `<span class="text-muted">${pText}</span></p></a>`;
                    }
                }
                if(--counter===0){
                    notifications.UI.general.append(notifications.cats.general.items);
                    notifications.UI.todo.append(notifications.cats.todo.items);
                    notifications.UI.financial.append(notifications.cats.financial.items);
                }
            }
        }
        setTimeout(getNotifications, 1500);
        $("body").on("click", "#right-bar .notify-item", function(event){
            let clicked = $(this);
            let data_user = $(this).data("user");
            ajaxCall ('notify', 'seen', {id: clicked.data("id")}, function(response){
                ...
                if (resObj.e) { // ERROR
                    ...
                    if(clicked.data("cat") === "general"){
                        // $(location).attr('href', 'leads.php');
                        toastr.success("Notification have been seen.");
                    }
                    else if(clicked.data("cat") === "financial") {
                        toastr.success("Notification have been seen.");
                    }
                    else if(clicked.data("cat") === "todo") {
                        const url = "user-edit.php?code="+data_user;
        ...
        $("body").on("click", "#right-bar .seen-all", function(event){
            let target = $(this).data("target");
            let ids = [];
            $(`#right-bar #${target}-notifications .notify-item`).each(function(){
                $(this).addClass( "foo" );
                ids.push( $( this ).data( "id" ) );
            }).after(function (){
                ajaxCall ('notify', 'seen', {id: ids.join(',')}, function(response){
                    console.log(response);
                    let resObj = JSON.parse(response);
                    if (resObj.e) { // ERROR
                        toastr.error("Error on saving form !");
                    } else if (resObj.res) { // SUCCESS
                        $(`#right-bar #${target}-notifications`).html('');
                        getNotifications(false);
                    }
                });
            });
    ...
    <script>
        ...
        /*  Add Uploader */
        $("body").on("change",'.multi-input .custom-file-input', function() {
